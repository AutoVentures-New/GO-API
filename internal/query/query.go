...
var ListEmailData = `SELECT record_number, ulid, account_id, message_id, thread_id, subject, eb.from, eb.to, cc, bcc, reply_to, headers, starred, unread, reply_to_message_id, body, files, folder, links, opens, link_clicks, is_tracked, eb.date, created_at, updated_at FROM tenant_%s.email_bucket eb`

var ListCallData = `SELECT record_number, ulid, created_by, user_phone_number, contact_phone_number, done, c.to, call_type, direction, outcome, notes, user_create_date, created_at, updated_at FROM tenant_%s.calls c`
