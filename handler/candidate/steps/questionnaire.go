...
type SaveQuestionnaireRequest struct {
	Questionnaire model.CandidateQuestionnaire `json:"questionnaire"`
}

func SaveQuestionnaire(fiberCtx *fiber.Ctx) error {
	candidate := fiberCtx.Locals("candidate").(model.Candidate)
	request := new(SaveQuestionnaireRequest)
	...
	if application.Status != model.FILLING ||
		(application.CurrentStep != model.QUESTIONNAIRE_BEHAVIORAL && application.CurrentStep != model.QUESTIONNAIRE_PROFESSIONAL) {
		return responses.BadRequest(fiberCtx, "Invalid step")
	}

	application, err = steps.SaveQuestionnaire(fiberCtx.UserContext(), application, request.Questionnaire)
	if err != nil {
		return responses.InternalServerError(fiberCtx, err)
	}
